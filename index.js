...
const getEndpointMethodName = endpoint => {
  return endpoint.split(/\//).reduce((accum, segment) => {
...
const request = (endpoint, agency) => (options = {}, callback) => {
  ...
  req.then(resp => callback(null, resp.body)).catch(callback)
}
const metro = ENDPOINTS.reduce(
  (accum, endpoint) => {
    Object.keys(agencies).forEach(agency => {
      if (typeof accum[agency] === 'undefined') {
        accum[agency] = {}
      }
      accum[agency][getEndpointMethodName(endpoint)] = request(
        endpoint,
        agencies[agency]
      )
    })
